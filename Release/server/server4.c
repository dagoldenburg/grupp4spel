...
int notReady(struct clientData *c,int threadLocal){
    int i;
    char number[2];
    ...
    for(i = 0;i<maxplayer;i++) // skicka til alla för att uppdatera alla om att en ny spelare finns
        if(c->clientControl[i]==1)
            if (send(c->client[i], nReady, sizeof(nReady), 0) == -1) {
                    perror("send");
            }
    }
    return;
...
int ready(struct clientData *c,int threadLocal){
    int i;
    ...
    for(i = 0;i<maxplayer;i++) // skicka til alla för att uppdatera alla om att en ny spelare finns
        if(c->clientControl[i]==1)
            if (send(c->client[i], ready, sizeof(ready), 0) == -1) {
                    perror("send");
            }
    }
    return;
    ...
    printf("Connected,clientControlToken = %d.\n",c->clientControl[threadLocal]);
        ...
        while(!done) {
            ...
            if (n <= 0) {
                    ...
                    if(strstr(c->Data[threadLocal].buffer,"disconnecting")!=NULL){
                    pthread_mutex_lock(&iMutex);
                    c->currentClient = threadLocal;
                    notReady(c,threadLocal);
                    c->clientControl[threadLocal]=0;
                    ...
                    if(strstr(c->Data[threadLocal].buffer, "plyrinf")!=NULL)
                        updatePlayerInfo(c);
                    if(strstr(c->Data[threadLocal].buffer, "plrrdy!")!=NULL){
                        ready(c,threadLocal);
                        if(c->readyPlayers>=1){
                        printf("Run mode!\n");
                            c->lobbyMode = 0;
                            char startgame[30]="strtgam.......................";
                            for(i = 0;i<maxplayer;i++) // skicka til alla för att uppdatera alla om att en ny spelare finns
                                if(c->clientControl[i]==1)
                                    if (send(c->client[i], startgame, sizeof(startgame), 0) == -1) {
                                            perror("send");
                                    }
                        }
                    }
                    if(strstr(c->Data[threadLocal].buffer, "plrnrdy")!=NULL){
                        notReady(c,threadLocal);
                    }
...
void sendAttack(struct clientData *c){
    ...
    for(i=0;i<maxplayer;i++){
        if(c->clientControl[i]==1)
            if(send(c->client[i], attackString, sizeof(attackString), 0) < 0) {
                perror("send");
...
void spawnAi(struct clientData *c,char x[], char y[],int *nrOfAi){
    int i;
    char spawnAiString[30];
    strcpy(spawnAiString,"spawnai, ID:000 x:0000 y:0000"); // identifikationsnumret på AIn man spawnar, static gör så att vid varje funktions call är värdet det samma.
    for(i=0;i<100;i++){
...
void* aiHandler(void *clientData){ //thread
    ...
    while(1){
        ...
        controlAi(c);
        time(&currentTime);
        if(currentTime>lastTime+10 && c->lobbyMode==0){
...
void stringManipulate(int intToString,int posInList,char* string){
    ...
    snprintf(intStrBufferMove, 7, "%d",intToString);
...
/*void* masterServer(void *clientData){
    ...
    struct clientData *c=(struct clientData *) clientData;
    char recvBuffer[30];
    char sendBuffer[30]="serverInfo: plr:0 rplr:0 play:0";
    if(recv(c->masterServerHandle,infoBuffer,sizeof(infoBuffer),NULL)==-1){
    ...
    if(strstr(infoBuffer,"listInfo")!=NULL){
        ...
        if(send(c->masterServerHandle,infoBuffer,sizeof(infoBuffer),0)<0){
            printf("Error in communication with master server\n");
        }
    }
}*/
...
int main(char argc ,char *argv[])
{
    ...
    /*masterServ_addr.sin_family = AF_INET;
    ...
    pthread_create(&clientData.clientThread[clientData.currentClient],NULL,&masterServer,(void *)&clientData);*/
    ...
    for(;;) {
        ...
        for(i=0;i<maxplayer;i++){
            ...
            if(slotsAvailable==0){
                printf("Server full\n");
                    if((clientData.errorHandle = accept(server, (struct sockaddr *)&client_address, &t)) == -1) {
                        perror("accept");
                    }
